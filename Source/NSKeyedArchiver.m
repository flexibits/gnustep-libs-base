  ...
  if (NSStringClass == 0)
    {
      ...
      scanFloatImp = (BOOL (*)(NSScanner*, SEL, CGFloat*))[NSScannerClass instanceMethodForSelector: scanFloatSel];
      scanStringImp = (BOOL (*)(NSScanner*, SEL, NSString*, NSString**))[NSScannerClass instanceMethodForSelector: scanStringSel];
      scannerImp = (id (*)(Class, SEL, NSString*))[NSScannerClass methodForSelector: scannerSel];
    }
}
...
#define CHECKKEY \
  ...
  if ([_enc objectForKey: aKey] != nil) \
    { \
      [NSException raise: NSInvalidArgumentException \
                  format: @"%@, duplicate key '%@' in %@", NSStringFromClass([self class]), aKey, NSStringFromSelector(_cmd)]; \
    }
          ...
          if (conditional == YES)
            {
              ...
              if (node == 0)
                {
                  ref = [_obj count];
                  GSIMapAddPair(_cIdMap, (GSIMapKey)anObject, (GSIMapVal)(NSUInteger)ref);
              ...
              if (node == 0)
                {
                   ...
                   */
                  ref = [_obj count];
                  GSIMapAddPair(_uIdMap, (GSIMapKey)anObject, (GSIMapVal)(NSUInteger)ref);
      ...
      if (node == 0)
        {
          ...
          ref = [_obj count];
          GSIMapAddPair(_uIdMap, (GSIMapKey)c, (GSIMapVal)(NSUInteger)ref);
...
+ (void) initialize
{
  ...
  if (globalClassMap == 0)
    {
      globalClassMap = NSCreateMapTable(NSNonOwnedPointerMapKeyCallBacks, NSObjectMapValueCallBacks, 0);
...
{
  id o = [[_NSKeyedCoderOldStyleArray alloc] initWithObjCType: aType
                                                       count: aCount
                                                          at: address];
  [self encodeObject: o];
  RELEASE(o);
  ...
  switch (*type)
    {
      ...
      case _C_ULNG_LNG:
        {
          o = [NSNumber numberWithUnsignedLongLong: *(unsigned long long*)address];
          [_enc setObject: o forKey: aKey];
          return;
        }
      ...
      case _C_STRUCT_B:
        {
          [NSException raise: NSInvalidArgumentException
                      format: @"-[%@ %@]: this archiver cannote encode structs", NSStringFromClass([self class]), NSStringFromSelector(_cmd)];
          return;
        }
...
- (void) encodeRect: (NSRect)aRect forKey: (NSString*)aKey
{
  ...
  val = [NSString stringWithFormat: @"{{%g, %g}, {%g, %g}}", aRect.origin.x, aRect.origin.y, aRect.size.width, aRect.size.height];
  [self encodeObject: val forKey: aKey];
}
  ...
  if (val == 0)
  ...
  else
    {
      ...
      if (!(*scanStringImp)(scanner, scanStringSel, @"{", NULL)
        ...
        {
          [NSException raise: NSInvalidArgumentException
                      format: @"[%@ -%@]: bad value - '%@'", NSStringFromClass([self class]), NSStringFromSelector(_cmd), val];
        }
    }
  return aPoint;
}
