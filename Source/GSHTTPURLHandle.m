...
+ (NSURLHandle *)cachedHandleForURL:(NSURL *)newUrl
{
    ...
    if ([s caseInsensitiveCompare:@"http"] == NSOrderedSame || [s caseInsensitiveCompare:@"https"] == NSOrderedSame) {
...
- (void)bgdApply:(NSString *)basic
{
    ...
    if ((id)NSMapGet(wProperties, (void *)@"Authorization") == nil) {
         ...
         * header.
         */
        if ([u user] != nil && (space = [GSHTTPAuthentication protectionSpaceForURL:u]) != nil) {
            NSString *auth;
...
- (void)_apply
{
    ...
    if (proxyURL && [[u scheme] isEqualToString:@"https"] == NO) {
        if ([u port] == nil) {
...
- (void)bgdConnect:(NSNotification *)notification
{
    ...
    if ([[u scheme] isEqualToString:@"https"]) {
        ...
        if (debug)
            [opts setObject:@"YES" forKey:GSTLSDebug];
...
- (BOOL)writeProperty:(id)property forKey:(NSString *)propertyKey
{
    ...
    if ([propertyKey hasPrefix:@"GSHTTPProperty"] || [propertyKey hasPrefix:@"GSTLS"] || [propertyKey hasPrefix:@"NSHTTPProperty"]) {
        if (property == nil) {
