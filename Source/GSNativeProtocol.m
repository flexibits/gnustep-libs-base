...
- (void)stopLoading
{
    ...
    if (nil != (task = [self task]) && NSURLSessionTaskStateSuspended == [task state]) {
        [self suspend];
    } else {
...
- (void)setInternalState:(GSNativeProtocolInternalState)newState
{
    ...
    if (isEasyHandleAddedToMultiHandle(_internalState) && !isEasyHandleAddedToMultiHandle(newState)) {
    ...
    if (!isEasyHandleAddedToMultiHandle(oldState) && isEasyHandleAddedToMultiHandle(_internalState)) {
        if (nil != (task = [self task])) {
...
- (void)resume
{
    ...
    if (_internalState == GSNativeProtocolInternalStateInitial) {
        ...
        if (nil != [self cachedResponse] && [self canRespondFromCacheUsing:[self cachedResponse]]) {
    ...
    if (_internalState == GSNativeProtocolInternalStateTransferReady && nil != _transferState) {
...
- (void)notifyDelegateAboutReceivedData:(NSData *)data
{
    ...
    if (nil != delegate && [task isKindOfClass:[NSURLSessionDownloadTask class]] && [delegate respondsToSelector:@selector(URLSession:
...
- (void)notifyDelegateAboutUploadedDataCount:(int64_t)count
{
    ...
    if (nil != delegate && [task isKindOfClass:[NSURLSessionUploadTask class]] && [delegate respondsToSelector:@selector(URLSession:task:didSendBodyData:totalBytesSent:totalBytesExpectedToSend:)]) {
