...
- (void)updatePauseState:(GSEasyHandlePauseState)pauseState
{
    ...
    bitmask = 0 | (send ? CURLPAUSE_SEND : CURLPAUSE_SEND_CONT) | (receive ? CURLPAUSE_RECV : CURLPAUSE_RECV_CONT);
...
- (void)setCookiesWithHeaderData:(NSData *)data
{
    ...
    if (nil != _config && NSHTTPCookieAcceptPolicyNever != [_config HTTPCookieAcceptPolicy] && nil != [_config HTTPCookieStorage]) {
