...
- (NSURL*) standardizedURL
{
  ...
  unsigned len = strlen(url);
  NSString *str = [[NSString alloc] initWithCStringNoCopy: url
                                                   length: len
                                             freeWhenDone: YES];
  NSURL *tmp = [NSURL URLWithString: str];
...
- (NSArray *) percentEncodedQueryItems
{
  ...
  if (internal->_queryItems != nil)
    {
      NSMutableArray *items = [[NSMutableArray alloc] initWithCapacity: [internal->_queryItems count]];
      NSEnumerator *en = [internal->_queryItems objectEnumerator];
      ...
      while ((i = [en nextObject]) != nil)
        {
          ...
          NSURLQueryItem *ni = [NSURLQueryItem queryItemWithName: name value: value];
